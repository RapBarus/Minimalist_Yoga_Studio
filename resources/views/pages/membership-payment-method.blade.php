    ...
    <div class="page">
        ...
        <div class="content">
            ...
            <div class="order-card">
                <div class="order-title">{{ $package->name }}</div>
                <div class="order-meta">
                    {{ $package->class_name ?? '' }} &nbsp;·&nbsp;
                    {{ $package->quota_amount }}x Pertemuan &nbsp;·&nbsp;
                    {{ $package->validity_months * 30 }} Hari
                </div>
                <div class="order-price">Rp {{ number_format($package->price, 0, ',', '.') }}</div>
            </div>

            <div class="section-title">Pilih Metode Pembayaran</div>

            <form id="payment-form" action="{{ route('membership.payment.method.process', $package->package_id) }}"
                method="POST">
                @csrf
