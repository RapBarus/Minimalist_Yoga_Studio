...
@push('styles')
    <style>
        .back-btn {
            ...
            color: var(--clay); text-decoration: none; font-size: .78rem;
        ...
        .info-field label { display: block; font-size: .68rem; font-weight: 700; letter-spacing: .1em; text-transform: uppercase; color: var(--text-muted); margin-bottom: 5px; }
        .info-field-wrap { display: flex; align-items: center; gap: 8px; }

        .info-field input,
        .info-field select,
        .info-field textarea {
            flex: 1; padding: .7rem .9rem; background: var(--bg-white);
        ...
        .info-field textarea:focus { border-color: var(--clay); }

        .btn-field-edit {
            background: none; border: none; cursor: pointer; padding: 6px;
            border-radius: 8px; color: var(--clay); flex-shrink: 0; transition: background .15s;
        }
        .btn-field-edit:hover { background: var(--clay-pale); }
        .btn-field-edit svg { width: 15px; height: 15px; stroke: currentColor; fill: none; stroke-width: 2; stroke-linecap: round; }

        .date-range-wrap { display: flex; align-items: center; gap: 8px; margin-bottom: 14px; }
        .date-range-wrap input[type="date"] {
            flex: 1; padding: .6rem .8rem; background: var(--bg-white);
            border: 1.5px solid var(--border); border-radius: 10px;
            font-family: 'Raleway', sans-serif; font-size: .82rem;
            color: var(--text); outline: none; transition: border-color .2s;
        }
        .date-range-wrap input[type="date"]:focus { border-color: var(--clay); }
        .date-range-sep { font-size: .8rem; color: var(--text-muted); flex-shrink: 0; }

        .btn-filter {
            padding: .6rem 1rem; background: var(--clay); color: #fff;
            border: none; border-radius: 10px; font-family: 'Raleway', sans-serif;
            font-size: .75rem; font-weight: 600; cursor: pointer;
            transition: background .18s; flex-shrink: 0;
        }
        .btn-filter:hover { background: var(--clay-dark); }

        .history-table { width: 100%; border-collapse: collapse; font-size: .82rem; }
        .history-table thead tr { background: var(--clay); color: #fff; }
        .history-table th { padding: 10px 14px; text-align: left; font-size: .7rem; font-weight: 600; letter-spacing: .08em; text-transform: uppercase; }
        ...
        .history-table tr:hover td { background: #faf8f6; }

        .pendapatan-box { background: var(--bg-white); border: 1.5px solid var(--border); border-radius: 14px; padding: 20px; }
        .pendapatan-label { font-size: .7rem; font-weight: 700; letter-spacing: .1em; text-transform: uppercase; color: var(--text-muted); margin-bottom: 8px; }
        .pendapatan-value { font-family: 'Cormorant Garamond', serif; font-size: 1.8rem; font-weight: 600; color: var(--clay); }

        .btn-delete-coach {
            width: 100%; padding: .85rem; background: var(--danger); color: #fff;
            border: none; border-radius: 12px; font-family: 'Raleway', sans-serif;
            font-size: .82rem; font-weight: 600; letter-spacing: .1em;
            text-transform: uppercase; cursor: pointer; transition: background .18s;
        }
        .btn-delete-coach:hover { background: #a93226; }

        .section-label-sm { font-size: .68rem; font-weight: 700; letter-spacing: .12em; text-transform: uppercase; color: var(--text-muted); margin-bottom: 10px; }
    </style>
@endpush
...
@section('content')
    <div class="content">
        ...
        <form action="{{ route('admin.coaches.update', $coach->coach_id) }}" method="POST">
            ...
            {{-- FIXED: was <select name="specialization"> comparing $coach->specialization --}}
            <div class="info-field">
